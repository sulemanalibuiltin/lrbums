...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("homepage").scrollTop($("select").find("option[value={{$role->homepage_id}}]").offset().top);
                ...
                <div class="col-md-12">
                    <div class="panel panel-primary">
                        ...
                        <div class="panel-body">
                            ...
                            {!! Form::open(['class' => 'form-horizontal', 'id' => 'role_form' ,'method' => 'put', 'route' => ['roles.update', $role->id]]) !!}
                            <fieldset class="content-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-lg-10">
                                        ...
                                        {!!   Form::textarea('description', $role->description, ['size' => '5x5', 'class' => 'form-control', 'placeholder' => '']) !!}
                                    </div>
                                </div>
                                ...
                                <div class="form-group{{ $errors->has('checked_modules') ? ' has-error' : '' }}">
                                    ...
                                    <input type="hidden" value="{{ implode(',', $assigned_modules) }}" id="assigned_modules" name="assigned_modules">
                                    <label class="control-label col-lg-2">Assign Modules</label>
                                    <div class="col-lg-10">
                                        <div class="tree-checkbox-hierarchical well"
                                             {!! $errors->first('checked_modules') ? 'style="border: 1px solid #d84315;"' : ''!!} id="role_tree">
                                            ...
                                            <ul>
                                                @foreach($modules as $module)
                                                    ...
                                                    @if($module->parent_id == 0)
                                                        <li id="{{$module->id}}"
                                                            class="{{ (in_array($module->id, $assigned_modules)? 'selected': '') }} folder expanded" >{{ $module->title }}
                                                            <ul>
                                                                @foreach($modules as $cmodule)
                                                                    @if($cmodule->parent_id == $module->id)
                                                                        <li id="{{$cmodule->id}}"
                                                                            class="{{ (in_array($cmodule->id, $assigned_modules)? 'selected': '') }}"
                                                                        >{{ $cmodule->title }}</li>
                                                                    @endif
                                                                @endforeach
